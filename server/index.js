...
class FydeDropServer {
    constructor(port) {
        const WebSocket = require('ws');
        this._wss = new WebSocket.Server({ port: port });
        this._wss.on('connection', (socket, request) => this._onConnection(new Peer(socket, request)));
        this._wss.on('headers', (headers, response) => this._onHeaders(headers, response));
        this._wss.on('error', (error) => this._onError(error));

        this._rooms = {};

        console.log('FydeDrop node server is running on port', port);
    }

    _onConnection(peer) {
        console.log('_onConnection');
        console.log(peer.ip, peer.id);
        this._joinRoom(peer);
...
class Peer {
    ...
    _setName(req) {
        ...
        // FydeOS
        if (this.name.os.includes('Chromium')) {
            this.name.os = 'FydeOS';
        }
...
const server = new FydeDropServer(process.env.PORT || 6000);
